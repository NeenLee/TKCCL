...
import pandas as pd
import tqdm
from loadRemap2ViewData import KGLoader, StructureLoader
...
def find_Max_Min_mod(cursor):
    ...
    cursor.execute('select asin from Cell_Phones_and_Accessories_2010')
    asins = cursor.fetchall()
    for asin in asins:
        product_set.add(asin[0])
    for pro in tqdm.tqdm(product_set):
        cursor.execute("select Overall, UnixReviewTime from Cell_Phones_and_Accessories_2010 where asin=?",
                       (pro,))
        row = cursor.fetchall()
...
class struct_kg_views:
    def __init__(self, database_path):
        ...
        self.file_path = '../Data/Cell_Phones_and_Accessories/2010/'
...
if __name__ == '__main__':
    ...
    pretrain_embedding_path = '../pretrain/Cell_Phones_and_Accessories/2010/cp.npz'

    detected_path = result_path + 'user_item_anomalyScore_14.csv'  # detected_spammer_path
    parm_path = result_path + 'parm_precision_14.csv'
    parm_precision_result = {'mod': result_mod, 'precision': []}
    num_parm = 1
    ...
    for mod in result_mod:
        ...
        os.system('python ../LNModel/main.py' if os.name == 'nt' else 'clear')
        precision, result = read_result(rp, detected_path)
