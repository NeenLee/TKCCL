...
# from TSGD.output.read_result import read_result
...
def find_Max_Min_mod(cursor):
    ...
    # cursor.execute('select prod_id from metadata13_14')
    cursor.execute('select asin from Cell_Phones_and_Accessories_2014')
    asins = cursor.fetchall()
    for asin in asins:
        product_set.add(asin[0])
    for pro in tqdm.tqdm(product_set):
        # cursor.execute("select Overall, strftime('%s', date) from Cell_Phones_and_Accessories_2010 where asin=?",
        #                (pro,))
        cursor.execute(
            "select Overall, unixReviewTime from Cell_Phones_and_Accessories_2014 where asin=?",
            (pro,))
        row = cursor.fetchall()
...
class struct_kg_views:
    def __init__(self, database_path):
        ...
        self.file_path = '../Data/Cell_Phones_and_Accessories/2014/'
    ...
    def Struct_KG_view(self, time_session_threshold):
        ...
        product_set = set()
        self.cursor.execute('select asin from Cell_Phones_and_Accessories_2014')
        ...
        for pro in product_set:
            ...
            split_indices = []
            # self.cursor.execute(
            #     "select rating, strftime('%s', date), user_id from metadata13_14 where pro_id=?",
            #     (pro,))
            self.cursor.execute(
                "select  Overall, UnixReviewTime, reviewerID from Cell_Phones_and_Accessories_2014 where asin=? ",
                (pro,))
...
if __name__ == '__main__':
    ...
    test = struct_kg_views('G:\\DB\\amazonReviews.db')
    print("differences the Product sequences...")
    normal_mod_list_all = test.Struct_KG_view(1)
    ...
    pretrain_embedding_path = '../pretrain/Cell_Phones_and_Accessories/2014/cp.npz'

    detected_path = result_path + 'user_item_anomalyScore_0.7_0.csv'  # detected_spammer_path
    parm_path = result_path + 'parm_precision_temperature_0.7_0.csv'
    parm_precision_result = {'mod': result_mod, 'precision': [], 'recall': [], 'f1': []}
    num_parm = 1
    test.Struct_KG_view(1)
    ...
    #     precision, recall, f1, result = read_result(rp, detected_path)
