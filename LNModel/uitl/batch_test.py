import csv

import pandas as pd
from pyparsing import results

import TSGD.LNModel.uitl.metrics as metrics
from TSGD.LNModel.uitl.parser import parse_args
import multiprocessing
import heapq
import numpy as np
import torch
import torch.nn.functional as F
from TSGD.LNModel.uitl.loader_kgat import KGAT_loader

cores = multiprocessing.cpu_count() // 2

args = parse_args()
Ks = eval(args.Ks)

data_generator = KGAT_loader(args=args, path=args.data_path + args.dataset)
batch_test_flag = False

USR_NUM, ITEM_NUM = data_generator.n_users, data_generator.n_items
N_TRAIN, N_TEST = data_generator.n_train, data_generator.n_test
BATCH_SIZE = args.batch_size


# 这是需要改动的地方！

def ranklist_by_heapq(user_pos_test, test_items, rating, Ks):
    item_score = {}
    for i in test_items:
        item_score[i] = rating[i]

    K_max = max(Ks)
    K_max_item_score = heapq.nlargest(K_max, item_score, key=item_score.get)

    r = []
    for i in K_max_item_score:
        if i in user_pos_test:
            r.append(1)
        else:
            r.append(0)
    auc = 0.
    return r, auc


def get_auc(item_score, user_pos_test):
    item_score = sorted(item_score.items(), key=lambda kv: kv[1])
    item_score.reverse()
    item_sort = [x[0] for x in item_score]
    posterior = [x[1] for x in item_score]

    r = []
    for i in item_sort:
        if i in user_pos_test:
            r.append(1)
        else:
            r.append(0)
    auc = metrics.auc(ground_truth=r, prediction=posterior)
    return auc


def ranklist_by_sorted(user_pos_test, test_items, rating, Ks):
    item_score = {}
    for i in test_items:
        item_score[i] = rating[i]

    K_max = max(Ks)
    K_max_item_score = heapq.nlargest(K_max, item_score, key=item_score.get)

    r = []
    for i in K_max_item_score:
        if i in user_pos_test:
            r.append(1)
        else:
            r.append(0)
    auc = get_auc(item_score, user_pos_test)
    return r, auc


def get_performance(user_pos_test, r, auc, Ks):
    precision, recall, ndcg, hit_ratio = [], [], [], []

    for K in Ks:
        precision.append(metrics.precision_at_k(r, K))
        recall.append(metrics.recall_at_k(r, K, len(user_pos_test)))
        ndcg.append(metrics.ndcg_at_k(r, K, user_pos_test))
        hit_ratio.append(metrics.hit_at_k(r, K))

    return {'recall': np.array(recall), 'precision': np.array(precision),
            'ndcg': np.array(ndcg), 'hit_ratio': np.array(hit_ratio), 'auc': auc}


def test_one_user(x):
    # user u's ratings for user u
    rating = x[0]
    # uid
    u = x[1]
    # user u's items in the training set
    try:
        training_items = data_generator.train_user_dict[u]
    except Exception:
        training_items = []
    # user u's items in the test set
    user_pos_test = data_generator.test_user_dict[u]

    all_items = set(range(ITEM_NUM))

    test_items = list(all_items - set(training_items))

    if args.test_flag == 'part':
        r, auc = ranklist_by_heapq(user_pos_test, test_items, rating, Ks)
    else:
        r, auc = ranklist_by_sorted(user_pos_test, test_items, rating, Ks)

    return get_performance(user_pos_test, r, auc, Ks)


def LNtest(g, kg, model, users_to_test):
    model.eval()
    pool = multiprocessing.Pool(cores)
    u_batch_size = BATCH_SIZE
    test_users = users_to_test
    n_test_users = len(test_users)
    n_user_batchs = n_test_users // u_batch_size + 1
    user_id = list()
    for u_batch_id in range(n_user_batchs):
        start = u_batch_id * u_batch_size
        end = (u_batch_id + 1) * u_batch_size
        """
        改这里
        """
        user_batch = test_users[start: end]

        item_batch = range(ITEM_NUM)
        with torch.no_grad():
            score, embedding = model("test", g, kg)
            user = embedding[user_batch]
            item = embedding[item_batch + data_generator.n_users]
            rate_batch = torch.mm(user, torch.transpose(item, 0, 1)).detach().cpu().numpy()
        user_batch_rating_uid = zip(rate_batch, user_batch)
        for user_b in user_batch_rating_uid:
            user_id.append(user_b[1])
        batch_result = pool.map(test_one_user, user_batch_rating_uid)
    score_list = list()
    score = score.cpu().numpy()
    for i in range(0, len(user_id)):
        score_list.append(score[i])
    """
    这里的user与score是一一对应的
    """
    # print(len(user_id))
    # print(len(score_list))
    # with open("../output/user_item_anomalyScore_14.csv", "w") as csvfile:
    #     writer = csv.writer(csvfile)
    #     writer.writerow(["user_id", "score"])
    #     writer.writerows([list(user_id), list(score_list)])
    # print('user_batch_rating_uid = ', user_batch_rating_uid)
    # score 的长度不对，其是user+item的长度
    # print('anomaly score = ', score)
    # print('batch_result', batch_result)
    return user_id, score_list


def test(g, kg, model, users_to_test):
    model.eval()
    result = {'precision': np.zeros(len(Ks)), 'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks)),
              'hit_ratio': np.zeros(len(Ks)), 'auc': 0.}

    pool = multiprocessing.Pool(cores)

    u_batch_size = BATCH_SIZE

    test_users = users_to_test
    n_test_users = len(test_users)
    n_user_batchs = n_test_users // u_batch_size + 1

    count = 0

    for u_batch_id in range(n_user_batchs):
        start = u_batch_id * u_batch_size
        end = (u_batch_id + 1) * u_batch_size
        """
        改这里
        """
        user_batch = test_users[start: end]

        item_batch = range(ITEM_NUM)
        with torch.no_grad():
            embedding = model("test", g, kg)
            user = embedding[user_batch]
            item = embedding[item_batch + data_generator.n_users]
            rate_batch = torch.mm(user, torch.transpose(item, 0, 1)).detach().cpu().numpy()

        user_batch_rating_uid = zip(rate_batch, user_batch)
        batch_result = pool.map(test_one_user, user_batch_rating_uid)
        count += len(batch_result)
        """
        改这里
        """
        for re in batch_result:
            result['precision'] += re['precision'] / n_test_users
            result['recall'] += re['recall'] / n_test_users
            result['ndcg'] += re['ndcg'] / n_test_users
            result['hit_ratio'] += re['hit_ratio'] / n_test_users
            result['auc'] += re['auc'] / n_test_users

    assert count == n_test_users
    pool.close()
    return result


def save_file(g, e_feat, model, users_to_test):
    model.eval()

    u_batch_size = BATCH_SIZE

    test_users = users_to_test
    n_test_users = len(test_users)
    n_user_batchs = n_test_users // u_batch_size + 1

    res = []

    for u_batch_id in range(n_user_batchs):
        start = u_batch_id * u_batch_size
        end = (u_batch_id + 1) * u_batch_size

        user_batch = test_users[start: end]

        item_batch = range(ITEM_NUM)
        with torch.no_grad():
            embedding = model(g, e_feat)
            user = embedding[user_batch]
            item = embedding[item_batch + data_generator.n_users]
            rate_batch = torch.mm(user, torch.transpose(item, 0, 1)).cpu().numpy()
            res.append(rate_batch)

    res = np.concatenate(res, axis=0)
    np.savetxt('{}_test_rate.txt'.format(args.dataset), res, fmt="%.06f")


def test_saved_file(users_to_test):
    res = np.loadtxt('{}_test_rate.txt'.format(args.dataset))
    result = {'precision': np.zeros(len(Ks)), 'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks)),
              'hit_ratio': np.zeros(len(Ks)), 'auc': 0.}

    pool = multiprocessing.Pool(cores)

    u_batch_size = BATCH_SIZE * 2

    test_users = users_to_test
    n_test_users = len(test_users)
    n_user_batchs = n_test_users // u_batch_size + 1

    count = 0

    for u_batch_id in range(n_user_batchs):
        start = u_batch_id * u_batch_size
        end = (u_batch_id + 1) * u_batch_size

        user_batch = test_users[start: end]

        item_batch = range(ITEM_NUM)
        rate_batch = res[start:end]

        user_batch_rating_uid = zip(rate_batch, user_batch)
        batch_result = pool.map(test_one_user, user_batch_rating_uid)
        count += len(batch_result)

        for re in batch_result:
            result['precision'] += re['precision'] / n_test_users
            result['recall'] += re['recall'] / n_test_users
            result['ndcg'] += re['ndcg'] / n_test_users
            result['hit_ratio'] += re['hit_ratio'] / n_test_users
            result['auc'] += re['auc'] / n_test_users

    assert count == n_test_users
    pool.close()
    return result
