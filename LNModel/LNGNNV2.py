...
class myGAT(nn.Module):
    ...
    def calc_contrastive_loss(self, embedding, user_id, pos_item, neg_item):
        user_emb = embedding[user_id]
        pos_emb = embedding[pos_item]
        neg_emb = embedding[neg_item]

        pos_dist = F.pairwise_distance(user_emb, pos_emb)
        neg_dist = F.pairwise_distance(user_emb, neg_emb)

        contrastive_loss = F.softplus(pos_dist - neg_dist).mean()
    ...
    def calc_cf_loss(self, g, sub_g, kg, user_id, pos_item, neg_item):  # (self, g, user_id, item_id, pos_mat):
        ...
        contrastive_loss = self.calc_contrastive_loss(embedding, user_id, pos_item, neg_item)
        return contrastive_loss
    ...
    def calc_cl_loss(self, g, kg, item):
        ...
        cf_emb = embedding_cl[item]
        cl_loss = self.calc_contrastive_loss(cf_emb, item, item + 1, item + 2)  # 具体实现可以根据实际情况调整
        return self.cl_alpha * cl_loss
    ...
    def calc_kg_loss(self, g, h, r, pos_t, neg_t):
        ...
        base_loss = F.softplus(-neg_score + pos_score).mean()
        return base_loss
